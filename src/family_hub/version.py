"""The hub's version — single source of truth.

`VERSION` (repo root) holds one SemVer string. `read_version()` reads it for
`family_hub.__version__` and the `/api/version` debug endpoint. The changelog
...
# repo root = two levels up from this file (src/family_hub/version.py). Tests
