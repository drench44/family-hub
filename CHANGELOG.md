...
- Seasonal looks. Turn **Season** on in the gear menu and the wall follows the
  calendar. Fall (Sep 1 to Nov 30) ships with three looks, each a quiet
  landscape behind the cards with its own palette, accent color, and a small
  mark beside the wordmark. **Harvest** is rolling farmland at dusk, **Maple**
  has a few maple leaves drifting down behind the cards, and **Woodland** is
  misty ridges of spruce. Light and Soft get a daytime version of each look;
  Blue, Grey and Black get the dusk version. Each device picks its look from
  ...
  night and stop for reduced motion, the cards stay solid so nothing gets
  harder to read, and all the art ships inside the app. Halloween, Christmas
  and the rest slot into the same registry in `theme.js`.
- The ridge art is generated by `scripts/gen-season-art.mjs`, and a test
