...
The pure helpers in common.js (time/label/severity math) are real logic that
deserves real execution, not just static grep contracts. There is no Node on
the box, so the runner falls back to a disposable node:20-alpine container
(docker IS on the box); locally a plain `node` is used when available. If
...
FH_ROOT = Path(__file__).resolve().parents[1]
JS_DIR = Path(__file__).resolve().parent / "js"


def _command(rel_files):
    """Explicit file list — `node --test <dir>` is flaky across Node majors.
    TAP reporter forced: Node 24 switched the piped-output default to the spec
    reporter, which breaks the `# fail 0` summary assertions below."""
    if shutil.which("node"):
        return ["node", "--test", "--test-reporter=tap",
                *[str(FH_ROOT / f) for f in rel_files]]
    if shutil.which("docker"):
        return [
            "docker", "run", "--rm",
            "-v", f"{FH_ROOT}:/fh:ro", "-w", "/fh",
...
def test_js_suite_passes():
    rel_files = sorted(str(p.relative_to(FH_ROOT)) for p in JS_DIR.glob("*.test.mjs"))
