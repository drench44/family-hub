"""family_hub.version.read_version — the single-source-of-truth SemVer reader.

Behind family_hub.__version__ and the /api/version debug endpoint. Never raises
(a partial deploy or a non-UTF-8 locale must not break import); explicit UTF-8
so it decodes identically regardless of the box's locale.
"""
import family_hub.version as fv
...
def test_read_version_survives_undecodable_file(tmp_path, monkeypatch):
    ...
    kill the app at import (family_hub.__version__)."""
