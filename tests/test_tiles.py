...
def test_laundry_cache_ttl_matches_watcher_cadence():
    # One flat TTL, sized just under the background watcher's 5s cadence
    # (app.LAUNDRY_WATCH_S): each watcher tick reaches HA while any request
    # landing between ticks reuses the still-warm cache. The old two-speed
    # endgame TTL is gone — the watcher IS the fast lane now, everywhere,
    # not just near a projected finish.
    ...
    ttl = tiles._laundry_cache["http://ha"][0] - _time.monotonic()
    assert ttl <= tiles.LAUNDRY_TTL + 0.5
    assert tiles.LAUNDRY_TTL <= 5.0, \
        "cache TTL must stay under the watcher cadence or ticks re-read a warm cache"
